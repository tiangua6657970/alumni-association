<template>
	<view id="waterfall_flow" class="waterfall-flow" :style="{'--pad':padpx,'--gap':gappx,'--col':columns}">
		<view class="item" v-for="(item,index) in list" :key="index" :style="{'--mbt':mbtpx,'--br':brpx,'grid-row':`span ${item.rows?item.rows:colWidth}`}" @click="clickItem(index)">
			<waterfall-flow-image :image="item[imageKey]" imgMode="widthFix" :height="item.imageHeight" :index="index" :border-radius="imageBR" @finish="loadImage($event,item)" @click="clickImage"></waterfall-flow-image>
			<view class="char" :style="{'--h':charSizeFn}" v-if="showChar && item.rows && item.showChar">
				<view class="title">{{item.title}}</view>
				<view class="product-info">
          <text class="aa-font-price">{{item.desc}}</text>
          <view class="icon icon--selected" @click.stop="$emit('iconClick', item)" v-if="item.isAddedToCart">
            <u-icon custom-prefix="custom-icon" name="shop-cart" size="30" color="#fff"/>
          </view>
          <view class="icon icon--unselected" @click.stop="$emit('iconClick', item)" v-else>
            <u-icon name="shopping-cart" size="30" color="#2b85e4"/>
          </view>
        </view>
			</view>
		</view>
